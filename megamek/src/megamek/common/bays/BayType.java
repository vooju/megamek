...
public enum BayType implements ITechnologyDelegator {
    ...
    /**
     * Bays fall into three basic categories: CATEGORY_CARGO is for bulk transport and can be sized in fractional
     * increments (usually half ton). Capacity is the fraction of bay tonnage that is available for storage.
     * CATEGORY_INFANTRY calculates cost based on tonnage rather than unit capacity and has no cost for aerospace units.
     * CATEGORY_NON_INFANTRY calculates cost per cubicle. Capacity is the number of entities that can fit in a single
     * cubicle.
     *
    ...
    /**
     * @return The capacity of a single unit of the bay type. For cargo the is the number of tons of cargo that can be
